...
vendor/bin/pest tests/Feature/Auth/  # Run specific test directory
```

### Code Quality
```bash
vendor/bin/pint  # Laravel Pint code formatting (available via composer)
composer pint  # Alternative via composer script
```
...
- **User Model**: Standard Laravel authentication model with role-based permissions
- **MoneyCast** (`app/Casts/MoneyCast.php`): Custom cast for handling money values (stores as cents, displays as dollars)
- **Role Enum** (`app/Enums/Role.php`): User roles (PARENT, RELATIVE, CHILD) with different permission levels
...
- **AllItems Widget**: Role-based widget for PARENT/RELATIVE users to view and manage other users' items
- **Permission System**: Purchase/delivery tracking only visible to PARENT/RELATIVE roles for other users' items
...
- hidden (boolean): for surprise gifts - items added by PARENT/RELATIVE are hidden from item owner
- timestamps
...
### API Routes
- **Laravel Sanctum**: API authentication enabled
- **User Endpoint**: `/api/user` - returns authenticated user information
- **API Routes**: Defined in `routes/api.php` with sanctum middleware
...
- `app/Filament/Widgets/AllItems.php`: Role-based widget for managing other users' items
...
- Role-based permission system controls access to purchase/delivery tracking features
- Items can be hidden from owners (surprise gift functionality) via `hidden` field
